...
export async function DELETE(request: Request, context: RouteContext) {
  try {
    ...
    if (!id?.trim()) {
      return NextResponse.json({ error: "Leadership member id is required." }, { status: 400 });
    }

    const docRef = adminDb.collection(COLLECTIONS.leadership).doc(id.trim());
    ...
    await writeAuditLog({
      action: "leadership.archived",
      actorUid: actor.uid,
      actorRole: actor.role!,
      targetType: "leadership",
      targetId: id.trim(),
      message: `Archived leadership member "${previous.name}" via delete action`,
    });
